...
import { createReverseOrder, clearError, CreateReverseOrderRequest } from '../../../../lib/slices/orderSlice';

interface ReverseOrderFormData {
  // Order Details
  orderId: string;
  channel: string;
  reasonForReturn: string;

  // Customer Details (where to pickup)
  customer?: Customer;

  // Pickup Details (warehouse/return address)
...
const CreateReverseOrderPage: React.FC = () => {
  ...
  // Form data state
  const [formData, setFormData] = useState<ReverseOrderFormData>({
    orderId: '',
    channel: '',
    reasonForReturn: '',
    customer: undefined,
  ...
  // Handle input change
  const handleInputChange = (field: keyof ReverseOrderFormData, value: string) => {
    setFormData(prev => {
  ...
  const transformFormDataToBackend = (): CreateReverseOrderRequest => {
    ...
    return {
      ...
      reason_for_return: formData.reasonForReturn.trim(),
      order_items: formData.items.map(item => ({
        ...
        is_fragile: false, // ItemDetails doesn't have isFragile property for reverse orders
      })),
  ...
  const validateForm = () => {
    ...
    if (!formData.reasonForReturn.trim()) {
      errors.reasonForReturn = 'Reason for return is required';
    }
  ...
  const handleCreateOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const resultAction = await dispatch(createReverseOrder(backendData));

      if (createReverseOrder.fulfilled.match(resultAction)) {
        ToastService.success('Reverse order created successfully!');
        router.push('/orders/reverse');
      } else {
        ToastService.error(resultAction.payload as string || 'Failed to create reverse order');
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                Fill in the details to create a new reverse/return shipment
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                Unique identifier for your reverse order.
              </p>
            </div>
          </div>
          ...
          <div className="mt-6">
            ...
            <select
              ...
              className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white ${formErrors.reasonForReturn ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'
                }`}
              required
            >
            ...
            {formErrors.reasonForReturn && (
              <p className="text-red-500 text-sm mt-1">{formErrors.reasonForReturn}</p>
            )}
          </div>
        </div>
          ...
          >
            {loading ? (
              ...
              <>
                ...
                <span>Create & Manifest Reverse Order</span>
              </>
            )}
          </button>
        </div>
      ...
      <AddCustomerModal
        ...
        title="Add Customer (Pickup From)"
